/**
 * services/scoring.js
 * Pure scoring and classification functions ported from original CARS project
 * Handles conversion of raw answers to T-scores and risk classification
 * 
 * Teaching Note: This module contains ONLY pure functions (no side effects)
 * Input → Processing → Output (same input always gives same output)
 */

// Question data: 25 bilingual questions
const QUESTIONS = [
...
// Rating options for questions 1-24
...
// Yes/No options for question 25 (self-harm)
const SELF_HARM_OPTIONS = [
  { label: "No (0)", value: 0 },
  { label: "Yes (1)", value: 1 }
];

// Scale definitions: which questions belong to which subscale
const SCALES = [
  ...
  { key: 'total', label: 'Total', items: Array.from({ length: 24 }, (_, i) => i + 1) } // Questions 1-24
];

// T-Score conversion tables (raw score → T-score)
const T_SCORE_TABLE = {
...
const RISK_THRESHOLDS = {
  normalMax: 60,    // T-score <= 60: Normal/No Risk
  atRiskMax: 70     // T-score 61-70: At-risk; >= 71: High risk
};

const SELF_HARM_ITEM_INDEX = 24; // Zero-based index for question 25

/**
 * Compute raw scores for each subscale
 * @param {number[]} answers - Array of 25 answers (0-4 for Q1-24, 0-1 for Q25)
 * @returns {Object} Raw scores by scale key
 */
function computeRawScores(answers) {
  const raw = {};
  
  for (const scale of SCALES) {
    // Sum up the answers for items in this scale
    raw[scale.key] = scale.items.reduce((sum, itemId) => {
      return sum + (answers[itemId - 1] || 0); // itemId is 1-based, array is 0-based
    }, 0);
  }
...
/**
 * Convert a single raw score to T-score using lookup table
 * @param {string} scaleKey - Scale key (e.g., 'externalizing', 'total')
 * @param {number} rawScore - Raw score to convert
 * @returns {number|null} T-score or null if table not found
 */
function convertRawToTScore(scaleKey, rawScore) {
  const table = T_SCORE_TABLE[scaleKey];
  if (!table) return null;
  
  // Direct lookup if exact match
  if (table[rawScore] !== undefined) {
    return table[rawScore];
  }
  
  // If raw score is below table range, return lowest T-score
  const keys = Object.keys(table).map(Number).sort((a, b) => a - b);
  if (rawScore <= keys[0]) return table[keys[0]];
  
  // If raw score is above table range, return highest T-score
  if (rawScore >= keys[keys.length - 1]) return table[keys[keys.length - 1]];
  
  // Find the closest lower raw score
  let lower = keys[0];
  for (const k of keys) {
    if (k > rawScore) return table[lower];
    lower = k;
  }
 ...
 * @param {Object} rawScores - Raw scores by scale key
 * @returns {Object} T-scores by scale key
 */
function convertAllToTScores(rawScores) {
  const tScores = {};
  
  for (const key in rawScores) {
    tScores[key] = convertRawToTScore(key, rawScores[key]);
  }
...
/**
 * Determine risk level and interpretation based on T-scores
 * @param {Object} tScores - T-scores by scale key
 * @param {boolean} hasSelfHarm - Whether question 25 was marked "Yes"
 * @returns {Object} { riskLevel, interpretation }
 */
function determineRisk(tScores, hasSelfHarm) {
  const total = tScores.total;
  let riskLevel, interpretation;
  
  // Base classification on total T-score
  ...
  // Self-harm override: Automatically sets to High risk
  if (hasSelfHarm) {
    riskLevel = "High risk";
...
function calculateAssessmentResult(answers) {
  ...
  return {
    ...
    selfHarmFlagged: hasSelfHarm,
    calculatedAt: new Date().toISOString()
  };
}

module.exports = {
  // Constants (for use in views and validation)
  QUESTIONS,
  RATING_OPTIONS,
  SELF_HARM_OPTIONS,
  SCALES,
  RISK_THRESHOLDS,
  SELF_HARM_ITEM_INDEX,
